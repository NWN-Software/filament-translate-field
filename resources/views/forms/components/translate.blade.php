@php
    ...
    $childComponentsWithLocale = collect($getChildSchemas())->map(fn($container) => $container->getComponents());
    ...
    $livewireKey =
        ...
        \SolutionForest\FilamentTranslateField\Forms\Component\Translate::class .
        '.container';
@endphp
...
<div wire:ignore.self x-cloak x-data="{
    ...
    <input type="hidden" value="{{ collect($tabs)->map(static fn($tab) => $tab?->getId())->values()->toJson() }}"
        x-ref="tabsData" />
    <x-filament::tabs :contained="$isContained" :label="$getLabel()">
        @foreach ($tabs as $tab)
            ...
            <x-filament::tabs.item :alpine-active="'tab === \'' . $tabId . '\''" :badge="$tab->getBadge()" :badge-color="$tab->getBadgeColor()" :icon="$tab->getIcon()"
                :icon-position="$tab->getIconPosition()" :x-on:click="'tab = \'' . $tabId . '\''">
                {{ $tab->getLabel() }}
